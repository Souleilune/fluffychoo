...
export default function ProductCard({ product, index = 0 }: ProductCardProps) {
  ...
  return (
    <div 
      className={`group relative h-[380px] sm:h-[420px] md:h-[450px] rounded-3xl overflow-hidden cursor-pointer hover-${hoverType}`}
      style={{
    ...
    >
      ...
      <div className="relative w-full h-full bg-white shadow-xl group-hover:shadow-2xl transition-all duration-500 rounded-3xl overflow-hidden">
        ...
        <div className="relative h-[60%] sm:h-[62%] md:h-[65%] w-full overflow-hidden bg-gradient-to-br from-amber-50 to-orange-50">
          <Image
            src={product.image}
            alt={product.name}
            fill
        ...
        {/* Content Section */}
        <div className="relative h-[40%] sm:h-[38%] md:h-[35%] bg-white p-4 sm:p-5 md:p-6 flex flex-col justify-between">
          
          <div>
            <h3 className="text-lg sm:text-xl md:text-2xl font-bold text-amber-900 mb-1 sm:mb-2 transform group-hover:translate-x-2 transition-all duration-300 line-clamp-1">
              {product.name}
            </h3>
            
            <p className="text-xs sm:text-sm text-gray-600 line-clamp-2 leading-relaxed transform group-hover:translate-x-1 transition-all duration-300 delay-75">
              {product.description}
            </p>
          </div>
          ...
          <div className="flex items-center justify-between pt-3 sm:pt-4 border-t border-gray-100">
            <div className="flex items-baseline gap-1 sm:gap-2 transform group-hover:scale-105 transition-transform duration-300">
              <span className="text-2xl sm:text-3xl font-black text-amber-600 animate-pulse-subtle">
                ₱{product.price.toFixed(2)}
              </span>
              <span className="text-xs sm:text-sm text-gray-500 font-medium">each</span>
            </div>
            
            {/* Decorative animated dots */}
            <div className="flex gap-1.5 sm:gap-2">
              <div className="w-2 h-2 sm:w-3 sm:h-3 rounded-full bg-amber-400 animate-bounce-1" style={{ animationDelay: '0s' }} />
              <div className="w-2 h-2 sm:w-3 sm:h-3 rounded-full bg-yellow-400 animate-bounce-1" style={{ animationDelay: '0.2s' }} />
              <div className="w-2 h-2 sm:w-3 sm:h-3 rounded-full bg-orange-400 animate-bounce-1" style={{ animationDelay: '0.4s' }} />
            </div>
          </div>
        </div>
      </div>
